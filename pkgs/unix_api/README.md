...
Unlike the packages above, `package:unix_api` requires a native tool chain and
has a small amount of native code that cannot be tree shaken away. In exchange,
it works on all POSIX platforms that Dart supports.
...
The POSIX API is a defined in terms of source, not object compatibility. For
this reason `package:unix_api` uses native assets to compile a small C shim
library. This allows the C compiler to deal with macro expansions, platform-specific
struct layout, etc. And because `package:unix_api` uses native assets, it does
not require a native toolchain when used by a consumer.
...
The Dart bindings are generated by `package:ffigen`. The `tool/` directory
contains the script `generate.dart` which drives `ffigen` using several
`.yaml` files. The `.yaml` files in `pkgs/unix_api/` fall into two groups:
1. `functions.yaml` and `constants.yaml` are lists of the POSIX functions and
   constants that `package:unix_api` should implement. These files are used to
   generate `lib/src/functions.g.dart`, `lib/src/constants.g.dart`, and
   `src/libc.g.c`.
2. The remaining `ffigen.yaml` files are `package:ffigen` configuration
   files. They are used to generate the Dart FFI bindings to the C functions
   in `src/libc.g.c` and `src/bespoke.c`.

To regenerate the bindings, run:

```
dart run pkgs/unix_api/tool/generate.dart
```

Note that `lib/src/bespoke.dart` contains handwritten Dart FFI bindings.
